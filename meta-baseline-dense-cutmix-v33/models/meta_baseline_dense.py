...
class MetaBaseline(nn.Module):
    ...
    def forward(self, x_shot, x_query):
        ...
        if self.method == 'cos':
            ...
            Similarity,Q_S_List = self.dense_SCL_similarity(input1 ,input2,self.neighbor_k)# query,support
            logits = Similarity
...
#============ support挑选出 top5, query基于support最重要的特征加权 ,no 超参-》求和-》norm  ================#
    def dense_SCL_similarity(self,input1,input2,neighbor_k): # 仅计算local和global cos的sim
        ...
        for i in range(b):
            ...
            for j in range(way):
                support_curr_set =  support[j,:,:,:,:] #[shot,h,w,c]
                
                """
                ==== AttentionSimilarity forward ====
                features_a torch.Size([15, 640, 5, 5])
                features_b torch.Size([5, 640, 5, 5])
                ==== projection of features a ====
                features_a  torch.Size([15, 25, 640])
                query_a torch.Size([15, 25, 80])
                key_a torch.Size([15, 25, 80])
                value_a torch.Size([15, 25, 80])
                ==== projection of features b ====
                 features_b  torch.Size([5, 25, 640])
                query_b torch.Size([5, 25, 80])
                key_b torch.Size([5, 25, 80])
                value_b torch.Size([5, 25, 80])
                 
                ==== AttentionSimilarity contrast_a_with_b ====
                ==== 1) spatial alignement ====
                value_a torch.Size([1, 15, 25, 80])
                value_b torch.Size([5, 1, 25, 80])
                ==== Align features A ====
                att_scores torch.Size([5, 15, 25, 25])
                att_probs torch.Size([5, 15, 25, 25])
                att_probs torch.Size([5, 15, 25, 25])
                ==== (BatchA x BatchB x HW x HW) x (BatchA x 1 x HW x C) -> (BatchA x BatchB x HW x C) ====
                aligned_features_a torch.Size([5, 15, 25, 80])
                ==== Align features B ====
                att_scores torch.Size([5, 15, 25, 25])
                att_probs torch.Size([5, 15, 25, 25])
                att_probs torch.Size([5, 15, 25, 25])
                ==== (BatchA x BatchB x HW x HW) x (1 x BatchB x HW x C) -> (BatchA x BatchB x HW x C) ====
                aligned_features_b torch.Size([5, 15, 25, 80])
                ==== 2) compute the spatial similarity ====
                similarity torch.Size([5, 15, 25])
                similarity torch.Size([5, 15, 25])
                ====  compute the total spatial similarity ==== 
                spatial_cont_loss torch.Size([5, 15])
                contrast torch.Size([5, 15])
                """
                
                features_a = query.permute(0,3,1,2) # [q_num,h,w,c] -> [q_num,c,h,w]
                features_b = support_curr_set.permute(0,3,1,2) #[shot,h,w,c]-> [shot,c,h,w]
                
                contrast = attention(features_a, features_b) # [shot,q_num] 算出来的相似度是相互的
                
                sim_way = torch.sum(contrast,dim=0) # [q_num]
                sim[:,j]= sim_way
