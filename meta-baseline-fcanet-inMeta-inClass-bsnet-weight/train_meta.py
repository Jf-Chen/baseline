...
def main(config):
    ...
    aves_keys = ['tl', 'ta', 'tvl', 'tva', 'vl', 'va','r_dn4','r_cos']
    trlog = dict()
    for k in aves_keys:
        trlog[k] = []
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            #==========================================================================#
            logits,r_dn4,r_cos =  model(x_shot, x_query).view(-1, n_train_way)
            ...
            aves['ta'].add(acc)
            aves['r_dn4'].add(r_dn4)
            aves['r_cos'].add(r_cos)
        ...
        for name, loader, name_l, name_a in [
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                ...
                with torch.no_grad():
                    logits,r_dn4,r_cos = model(x_shot, x_query).view(-1, n_way)
                    loss = F.cross_entropy(logits, label)
                    acc = utils.compute_acc(logits, label)
