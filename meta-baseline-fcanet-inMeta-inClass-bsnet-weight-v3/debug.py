...
print("logits_cos_unview[0,0,:]",logits_cos_unview[0,0,:],"logits_dn4_unview[0,0,:]",logits_dn4_unview[0,0,:])
logits_cos_unview[0,0,:] tensor([0.6816, 0.4036, 0.4062, 0.5474, 0.5820], device='cuda:0',
       grad_fn=<SliceBackward0>) logits_dn4_unview[0,0,:] tensor([13.3586,  8.9643,  9.0234,  9.6951, 11.0557], device='cuda:0',
       grad_fn=<SliceBackward0>)
       
print("topk_value.size()",topk_value.size())       

print("torch.sum(topk_value).size()",torch.sum(topk_value).size()) 
