...
class MetaBaseline(nn.Module):
    ...
    def forward(self, x_shot, x_query):
        ...
        if self.method == 'cos':
            ...
            input1_batch = input1_batch.permute(0,2,1)
            input2_batch = input2_batch.permute(0,2,1)
        ...
        elif selt.method == "M2L_cos_match":
            print("method == match")
            ...
            input1_batch =  x_query_aft.contiguous().view(b*q_num,dimension,h*w).permute(0,2,1)
            input2_batch = x_shot_aft.contiguous().view(b*shot*way,dimension,h*w).permute(0,2,1) # [b,way,shot,c,h,w] -> [b*shot*way,h*w,dimension]
            
            Similarity,Q_S_List = self.match_similarity(input1_batch,input2_batch)# query,support
            logits = Similarity
        
        return logits # [batch,q_num,way]
        
#=================================  M2L_cos_match  ==================================#
    def match_similarity(): # 仅计算local和global cos的sim
        # 经过校准，输入必须是 dim在最后一维
