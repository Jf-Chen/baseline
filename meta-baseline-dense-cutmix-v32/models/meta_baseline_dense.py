...
class MetaBaseline(nn.Module):
    ...
    def __init__(self, encoder, encoder_args={},temp=10., temp_learnable=True,
        ...
        self.weight = torch.randn([5,5,1]).cuda()
    ...
    def forward(self, x_shot, x_query):
        ...
        if self.method == 'cos':
            ...
            Similarity,Q_S_List = self.linear_similarity(input1 ,input2,self.neighbor_k)# query,support
            logits = Similarity
        ...
        for i in range(b):
            ...
            for j in range(way):
                ...
                for k in range(shot):
                    ...
                    sim_way[:,k] =cos(query_pool,support_pool) #[q_num]
                    
                sim[:,j] = torch.mean(sim_way,dim=1) #[q_num]
